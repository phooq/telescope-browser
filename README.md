...
- The desktop shell can also dock local PTY-backed CLI panes for `codex`, `claude`, or another command. CLI panes can start the cursor picker and explicitly add the selected element or current page context to the terminal chat without receiving Telescope owner or grant tokens.
...
The desktop chrome also has a local CLI pane row for terminal agents such as `codex` or `claude`. Enter a command, choose `Right`, `Left`, or `Bottom`, and click `Open CLI`. The pane runs the command in a PTY, streams output into the docked pane, and provides `Pick`, `Add selection`, and `Add page` controls. `Pick` starts the same one-shot page element picker used by the SDK; after clicking an element in the browser page, `Add selection` writes a structured descriptor into the CLI chat with the page URL, tab ID, element reference ID, selector, role, label, text preview, and bounds. Set `TELESCOPE_AGENT_CLI=claude` to prefill the command with Claude Code instead of the default `codex`.
